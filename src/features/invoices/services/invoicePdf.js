 ...
 */
export async function generateInvoicePdf(invoice, order, sellerProfile) {
    ...
    const totalAmount = (order.amount ?? invoice.amount ?? 0) +
    ...
    const contactParts = [];
    if (companyPhone)
        contactParts.push(`SĐT: ${companyPhone}`);
    if (companyEmail)
        contactParts.push(`Email: ${companyEmail}`);
    if (companyWebsite)
        contactParts.push(`Website: ${companyWebsite}`);
