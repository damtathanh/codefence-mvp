...
import { INVOICE_STATUS } from "./invoiceTypes";
...
function generateInvoiceCode(order) {
    if (order.order_id) {
 ...
 */
export async function ensurePendingInvoiceForOrder(order) {
    if (!order.user_id)
        return;
 ...
 */
export async function markInvoicePaidForOrder(order) {
    if (!order.user_id)
        return;
    ...
    if (existing) {
        ...
        if (existing.status === INVOICE_STATUS.PAID ||
        ...
        await logOrderEvent(order.id, "ORDER_PAID", {
            amount: order.amount,
            paid_at: now,
        }, "invoice_service");
        return existing;
 ...
 */
export async function deleteInvoicesByOrderIds(userId, orderIds) {
 ...
 */
export async function getInvoiceByOrderId(orderId, userId) {
    if (!orderId || !userId)
        return null;
 ...
 */
export async function fetchInvoicesByUser(userId, page, pageSize, filters) {
 ...
 */
export async function invalidateInvoicePdfForOrder(orderId) {
    if (!orderId)
        return;
 ...
 */
export async function applyInvoiceRules(order) {
