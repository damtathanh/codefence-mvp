...
import { fetchOrdersByUser, updateOrder as updateOrderService, fetchOrderFilterOptions } from '../services/ordersService';
import { fetchActiveProducts } from '../../products/services/productsService';
export const useOrdersData = () => {
    ...
    const fetchOrders = useCallback(async (overridePage, overrideFilters) => {
        if (!user) {
            setOrders([]);
            setLoading(false);
            return;
        ...
        try {
            ...
            const { orders: ordersData, totalCount: count, error: ordersError } = await fetchOrdersByUser(user.id, targetPage, PAGE_SIZE, currentFilters);
            if (ordersError)
                throw ordersError;
    ...
    const applyLocalOrderPatch = useCallback((orderId, patch) => {
    ...
    const updateOrderLocal = async (orderId, updates) => {
        if (!user)
            return false;
        ...
        applyLocalOrderPatch(orderId, updates);
        try {
            // 3. API Call
            ...
            if (data) {
                ...
                if (nextStatus) {
                    setStatusOptions(prev => prev.includes(nextStatus) ? prev : [...prev, nextStatus].sort());
                }
            }
            return true;
        }
