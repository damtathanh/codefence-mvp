...
export const OrdersView = () => {
    ...
    const { orders, products, loading, error, refreshOrders, updateOrderLocal, totalCount, page, setPage, pageSize, statusOptions, paymentMethodOptions, searchQuery, setSearchQuery, statusFilter, setStatusFilter, riskScoreFilter, setRiskScoreFilter, paymentMethodFilter, setPaymentMethodFilter, dateFilter, setDateFilter, } = useOrdersData();
    const filteredOrders = orders;
    const { selectedIds, handleSelectAll, handleToggleSelect, clearSelection, } = useOrderSelection(filteredOrders);
    const totalPages = Math.max(1, Math.ceil(totalCount / pageSize));
    const { handleApprove, handleConfirmReject, handleMarkShipped, // aka Start Delivery
    handleMarkCompleted, handleProductCorrection, handleDeleteOrders, handleSimulatePaid, handleSendQrLink, handleSimulateConfirmed, handleSimulateCancelled, } = useOrderActions(updateOrderLocal, refreshOrders);
    ...
    const [rejectMode, setRejectMode] = useState('VERIFICATION_REQUIRED');
    ...
    const [pendingConfirmOrder, setPendingConfirmOrder] = useState(null);
    ...
    const handleSaveAddress = async () => {
        ...
        try {
            ...
            await logOrderEvent(selectedOrder.id, "ADDRESS_UPDATED", changes, "orders_page_sidepanel");
    ...
    const handleSimulateConfirmedClick = async (order) => {
        if (!user)
            return;
        try {
            // FE hook sẽ:
    ...
    const handleSimulateCancelledClick = (order) => {
        if (!user)
            return;
    ...
    const handleSendQrPaymentLinkClick = async (order) => {
        try {
            // Xác định risk
    ...
    const handleStartDeliveryFromPanel = async (order) => {
        await handleMarkShipped(order);
        closeSidePanel();
    };
    const handleMarkCompletedFromPanel = async (order) => {
        await handleMarkCompleted(order);
        closeSidePanel();
    };
    const handleSimulatePaidFromPanel = async (order) => {
    ...
    return (_jsxs("div", { className: "space-y-6 p-6 h-full flex flex-col min-h-0", children: [_jsxs(FilterBar, { searchValue: searchQuery, onSearch: setSearchQuery, searchPlaceholder: "Search orders...", children: [_jsx(MultiSelectFilter, { label: "Statuses", options: statusOptions.map(s => ({ value: s, label: s })), selectedValues: statusFilter, onChange: setStatusFilter }), _jsx(MultiSelectFilter, { label: "Risk Levels", options: [
                            { value: 'low', label: 'Low Risk' },
                            { value: 'medium', label: 'Medium Risk' },
                            { value: 'high', label: 'High Risk' },
                        ], selectedValues: riskScoreFilter, onChange: setRiskScoreFilter }), _jsx(MultiSelectFilter, { label: "Payment Methods", options: paymentMethodOptions.map(m => ({ value: m, label: m })), selectedValues: paymentMethodFilter, onChange: setPaymentMethodFilter }), _jsx("input", { type: "date", value: dateFilter, onChange: (e) => setDateFilter(e.target.value), className: "h-10 px-3 bg-[var(--bg-input)] border border-[var(--border-subtle)] rounded-lg text-[var(--text-main)]" }), _jsx("button", { type: "button", onClick: handleClearFilters, className: "text-sm text-[var(--text-muted)] hover:text-white", children: "Clear filters" }), _jsxs(Button, { onClick: () => {
                            setEditingOrder(null);
                            setIsAddOrderModalOpen(true);
                        }, children: [_jsx(Plus, { className: "w-4 h-4 mr-2" }), " Add Order"] })] }), _jsx(Card, { className: "flex-1 flex flex-col min-h-0", children: _jsx(OrderTable, { orders: orders, filteredOrders: filteredOrders, totalCount: totalCount, currentPage: page, pageSize: pageSize, totalPages: totalPages, selectedIds: selectedIds, onSelectAll: handleSelectAll, onToggleSelect: handleToggleSelect, onPageChange: setPage, onRowClick: handleRowClick, products: products, onProductCorrection: handleProductCorrection, onApprove: (orderId) => {
                        ...
                        setIsAddOrderModalOpen(true);
                    }, onDelete: handleDeleteAllClick, loading: loading }) }), _jsx(OrderSidePanel, { isOpen: isSidePanelOpen, onClose: closeSidePanel, order: selectedOrder, orderEvents: orderEvents, addressForm: addressForm, isAddressModified: isAddressModified, onAddressChange: handleAddressChange, onSaveAddress: handleSaveAddress, blacklistedPhones: blacklistedPhones, 
                // Handlers dùng cho SidePanel
                onApprove: handleApproveOrder, onReject: handleRejectOrder, onMarkDelivered: handleStartDeliveryFromPanel, onMarkCompleted: handleMarkCompletedFromPanel, onSimulateConfirmed: handleSimulateConfirmedClick, onSimulateCancelled: handleSimulateCancelledClick, onSimulatePaid: handleSimulatePaidFromPanel, onSendQrPaymentLink: handleSendQrPaymentLinkClick, onOrderUpdated: refreshOrders }), _jsx(AddOrderModal, { isOpen: isAddOrderModalOpen, onClose: () => {
                    ...
                    setIsAddOrderModalOpen(false);
                    setEditingOrder(null);
                }, editingOrder: editingOrder }), _jsx(RejectOrderModal, { isOpen: isRejectModalOpen, mode: rejectMode, reason: rejectReason, onModeChange: setRejectMode, onReasonChange: setRejectReason, onConfirm: onConfirmReject, onCancel: () => setIsRejectModalOpen(false), loading: rejectLoading }), pendingConfirmOrder && (_jsxs(_Fragment, { children: [_jsx(CustomerConfirmationModal, { isOpen: isConfirmationModalOpen, onClose: handleConfirmationModalClose, order: pendingConfirmOrder }), _jsx(CancellationReasonModal, { isOpen: isCancellationModalOpen, onClose: () => setIsCancellationModalOpen(false), onConfirm: handleCancellationConfirm, order: pendingConfirmOrder })] }))] }));
};
