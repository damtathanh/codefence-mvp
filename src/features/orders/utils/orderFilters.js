 ...
 */
export function isAllFilter(selected) {
    return !selected || selected.length === 0 || selected.includes('all');
}
/**
 * Check if an order's status matches the status filter
 */
export function matchesStatusFilter(orderStatus, statusFilter) {
    if (isAllFilter(statusFilter))
        return true;
 ...
 */
export function matchesPaymentMethodFilter(order, paymentMethods) {
    if (isAllFilter(paymentMethods))
        return true;
    if (!paymentMethods)
        return false;
 ...
 */
export function matchesRiskFilter(riskScore, riskBuckets) {
    if (isAllFilter(riskBuckets))
        return true;
    if (!riskBuckets)
        return false;
    const score = riskScore || 0;
    // Check if score falls into any selected bucket
    if (riskBuckets.includes('low') && score <= 30)
        return true;
    if (riskBuckets.includes('medium') && score > 30 && score <= 70)
        return true;
    if (riskBuckets.includes('high') && score > 70)
        return true;
    return false;
}
