...
export async function simulateCustomerConfirmed(order) {
    const now = new Date().toISOString();
    const qrExpiredAt = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();
    const { error } = await updateOrder(order.id, order.user_id, {
...
export async function simulateCustomerCancelled(order, reason) {
    const now = new Date().toISOString();
    const { error } = await updateOrder(order.id, order.user_id, {
...
export async function simulateCustomerPaid(order) {
    const now = new Date().toISOString();
