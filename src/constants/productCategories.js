...
export const ALL_CATEGORIES = PRODUCT_CATEGORIES.flatMap(group => group.categories);
...
export const getCategoryDisplayName = (slug) => {
    ...
    return category?.displayName || slug.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ');
};
